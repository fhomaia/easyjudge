...
// Divisão Non-Tumbling do IASF, All Girl — pedido do usuário
// (2026-09-23), fonte "IASF_Scoring_NT.pdf" (mesma data 7.28.2025 do
// documento L1-L7 que gerou os templates irmãos All Girl/Coed, ver
// 1785640000000-AddIasf2526TeamCheerAllGirlTemplate.ts). Conteúdo
// IDÊNTICO ao template All Girl "com tumbling" em todo o resto
// (Stunt/Pyramid/Toss/Jump/Dance/Creativity/Formations/Performance,
// conferido linha a linha contra o PDF — nenhuma divergência
// encontrada, template irmão não precisou de correção) — só o grupo
// "Tumbling" inteiro (Standing/Running Tumbling Difficulty + Tumbling
// Technique) não existe aqui — a nota "No Tumbling Allowed" (com a
// exceção de forward rolls/handstands/cartwheels/round offs pra
// stunting ou mudança de formação), que substitui a nota de tumbling
// individual do documento L1-L7, foi anexada à `description` do
// próprio template, não virou um critério pontuável falso (não tem
// pontuação associada). Deduções = as 9 padrão da IASF já usadas na
// plataforma (pedido do usuário).
export const IASF_TEAM_CHEER_ALL_GIRL_NT_TEMPLATE_ID =
  'c1d2e3f4-1111-4abc-9def-000000000001';
const TEMPLATE_ID = IASF_TEAM_CHEER_ALL_GIRL_NT_TEMPLATE_ID;
...
export class AddIasfTeamCheerAllGirlNonTumblingTemplate1790040000000
  implements MigrationInterface
{
  name = 'AddIasfTeamCheerAllGirlNonTumblingTemplate1790040000000';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      ...
      [
        TEMPLATE_ID,
        'Team Cheer (All Girl) — Non-Tumbling',
        'Official IASF scoring system for Team Cheer, Non-Tumbling divisions, All Girl (2026 season). No tumbling allowed — exception: forward rolls, handstands, cartwheels and round offs are allowed for stunting purposes or formation changes, and must follow appropriate level inversion rules. Team Majority reference tables intentionally omitted (not a scored criterion).',
        targetScore,
